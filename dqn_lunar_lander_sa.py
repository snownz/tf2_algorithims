...
from dqn_agent import DqnAttentionAgent
...
args.experiment = 'attn_s256x128_bs2048_adam2e6_per_normal_nonstep'
args.load = True
args.train = False
...
dqn = DqnAttentionAgent( args.state_dim, args.action_dim, args.buffer_size, args.batch_size, args.experiment, 256, 128, priorized_exp = True, gn = False )
    ...
    for cycle in range(args.epoch_cycles):
        for rollout in range(args.rollout_steps):
            ...
            if total_steps < args.start_timesteps:
                ...
                action = dqn.act( tf.convert_to_tensor([state], dtype=tf.float32), eps = args.train * 0.1 )
