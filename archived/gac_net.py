...
class IQNActor(tf.Module):
    def __init__(self, state_dim, action_dim, name):
        ...
        self.log_dir = 'logs/actor_{}'.format(name)
        run_number = 0
    ...
    def huber_quantile_loss(self, actions, target_actions, taus, weights):
        ...
        # delta is kappa in paper
        eltwise_loss = tf.math.abs(taus - I_delta) * eltwise_huber_loss[:,tf.newaxis] * weights
...
class AutoRegressiveStochasticActor(IQNActor):
    def __init__(self, state_dim, action_dim, name, n_basis_functions = 64):
        """
        ...
        Class Args:
            ...
            n_basis_functions (int): the number of basis functions
        """
        super(AutoRegressiveStochasticActor, self).__init__(state_dim, action_dim, name)
...
class StochasticActor(IQNActor):
    def __init__(self, state_dim, action_dim, name, n_basis_functions=64):
        ...
        """
        super(StochasticActor, self).__init__(state_dim, action_dim, name)
        self.module_type = 'StochasticActor'
        self.noise_embed_dim = 32 // action_dim
        ...
        self.merge_embedding_layer = Dense(
            32, activation= tf.keras.layers.LeakyReLU(alpha=0.01),
...
class Critic(tf.Module):
    ...
    def __init__(self, state_dim, action_dim, name):
        ...
        self.log_dir = 'logs/critic_{}'.format(name)
        run_number = 0
        while os.path.exists(self.log_dir + str(run_number)):
            run_number += 1
        self.log_dir = self.log_dir + str(run_number)
    ...
    def train(self, states, actions, rewards, next_states, terminal_mask, value, gamma,
        ...
        self.train_loss(tf.reduce_mean(loss1+loss2))
...
class Value(tf.Module):
    ...
    def __init__(self, state_dim, name):
        ...
        self.log_dir = 'logs/value_{}'.format(name)
